...
const LatestNews = () => {
  ...
  return (
    <div className="news-section w-[90%] max-w-[1200px] mx-auto py-16 mt-[100px] font-Gothic">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {visibleNews.map((news, idx) => (
          <motion.div
            ...
            className="p-4 border border-white/20 rounded-lg bg-black text-white flex flex-col h-full cursor-pointer"
          >
            <img
              src={news.image}
              alt={news.title}
            ...
            <div className="mt-4 flex justify-between items-center">
              <button
                className="text-orange-400 hover:text-orange-500 underline bg-transparent border-none cursor-pointer font-sans text-sm"
      ...
      {/* Navigation */}
      <div className="flex flex-col sm:flex-row justify-between items-center mt-8 gap-4 px-2 sm:px-4">
        <p className="text-sm text-gray-400 font-sans text-center sm:text-left">
          Showing {Math.min(currentIndex + 1, blogsData.length)}–{Math.min(currentIndex + itemsPerPage, blogsData.length)} of {blogsData.length} News
        </p>
        <div className="flex gap-4 justify-center sm:justify-end w-full sm:w-auto">
          <button
            onClick={handlePrev}
            disabled={currentIndex === 0}
            className="px-4 py-2 border rounded bg-amber-50 cursor-pointer disabled:opacity-40 hover:bg-gray-100 font-Gothic transition"
          >
            Prev
          </button>
          <button
