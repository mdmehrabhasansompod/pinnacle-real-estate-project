...
import { FiUser, FiPhone, FiMail, FiDollarSign, FiMapPin, FiLayers, FiMessageSquare } from 'react-icons/fi';
import { assets } from '../../assets/frontend_assets/assets';

const ContactForm = () => {
  const [formData, setFormData] = useState({
    name: '', phone: '', email: '', budget: '', location: '', service: '', message: '', terms: false
  });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (itiRef.current) setFormData(prev => ({ ...prev, phone: itiRef.current.getNumber() }));
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setFormData({ name:'', phone:'', email:'', budget:'', location:'', service:'', message:'', terms:false });
  ...
  return (
    ...
    >
      <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y:0, opacity:1 }} transition={{ duration:0.6 }} className="w-full max-w-5xl">
        <motion.h2
        ...
        >
          <img className='w-12 sm:w-16 h-12 sm:h-16 object-contain' src={assets.serviceContact} alt="icon" />
          <span className='font-Gothic'>Have a project in your mind?</span>
        </motion.h2>

        <motion.form onSubmit={handleSubmit} className="w-full bg-black bg-opacity-50 backdrop-blur-lg p-6 sm:p-10 rounded-2xl border border-gray-700 shadow-lg">
          {/* Row 1 */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
            {[{label:'Name', icon:<FiUser />, name:'name', type:'text'},
              {label:'Phone', icon:<FiPhone />, name:'phone', type:'phone'},
              {label:'Email', icon:<FiMail />, name:'email', type:'email'}].map((field,i)=>(
              <motion.div key={field.name} custom={i} initial="hidden" animate="visible" variants={fieldVariant}>
                <label className="block text-sm text-gray-300 mb-2">{field.label}</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400 pointer-events-none">{field.icon}</div>
                  {field.type==='phone'?(
                    <>
                      <div className="absolute inset-y-0 left-10 pl-2 flex items-center text-xl pointer-events-none">{countryFlag}</div>
                      <input ref={phoneRef} type="tel" name="phone" onBlur={()=>{if(itiRef.current)setFormData(prev=>({...prev,phone:itiRef.current.getNumber()}))}}
                        required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
            {[
              {label:'Budget', icon:<FiDollarSign />, name:'budget', type:'select', options:['','<$5k','$5k-$10k','$10k-$25k','$25k+']},
              {label:'Location', icon:<FiMapPin />, name:'location', type:'text'},
              {label:'Service', icon:<FiLayers />, name:'service', type:'select', options:['','Residential Construction','Custom Homes','Community Development','Renovations']}
            ].map((field,i)=>(
              <motion.div key={field.name} custom={i+3} initial="hidden" animate="visible" variants={fieldVariant}>
                <label className="block text-sm text-gray-300 mb-2">{field.label}</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400 pointer-events-none">{field.icon}</div>
                  {field.type==='text'?(
          ...
          {/* Message */}
          <motion.div custom={6} initial="hidden" animate="visible" variants={fieldVariant} className="relative mb-6">
          ...
          {/* Terms + Submit */}
          <motion.div custom={7} initial="hidden" animate="visible" variants={fieldVariant} className="flex flex-col sm:flex-row items-center justify-between">
            <label className="flex items-center mb-4 sm:mb-0">
              ...
              <span className="ml-3 text-gray-300 text-sm">
                I agree to <a href="#" className="text-orange-400 hover:text-orange-300">Terms</a> and <a href="#" className="text-orange-400 hover:text-orange-300">Privacy Policy</a>
              </span>
            </label>

            <button type="submit" disabled={isSubmitting||submitSuccess}
              className={`w-full sm:w-auto mt-4 sm:mt-0 px-8 py-3 rounded-xl font-bold text-white transition-all duration-300 ${isSubmitting||submitSuccess?'bg-green-600 hover:bg-green-700':'bg-purple-600 hover:bg-purple-700'}`}>
              {isSubmitting?'Processing...':submitSuccess?'Message Sent!':'Send Message'}
            </button>
          </motion.div>
        </motion.form>
      </motion.div>
